...
void Main(const v8::FunctionCallbackInfo<v8::Value> &args) {
	...
	FILE *in = fopen("../text_files/main.txt" , "r");
	FILE *out = fopen("../text_files/main-compress.bin" , "wb");
	...
	// out << (bit_line_code.size() % 8) << std::endl;
	...
	auto total = v8::Number::New(isolate , 10);
...
NODE_MODULE(compress , Initialize);